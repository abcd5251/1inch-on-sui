...
import { FusionService, SwapParams, QuoteParams } from "~~/services/fusion/FusionService";
...
export const useFusion = (config: UseFusionConfig) => {
  ...
  const getQuote = useCallback(async (params: QuoteParams): Promise<Quote | null> => {
    try {
      updateState({ isLoading: true, error: null });
  ...
  const createOrder = useCallback(async (params: SwapParams): Promise<OrderInfo | null> => {
    try {
      updateState({ isLoading: true, error: null });
  ...
  const getActiveOrders = useCallback(async (page: number = 1, limit: number = 10) => {
    try {
      updateState({ isLoading: true, error: null });
  ...
  const getOrdersByMaker = useCallback(async (
    makerAddress: string,
    page: number = 1,
    limit: number = 10
  ) => {
  ...
  const getAddressFromPrivateKey = useCallback((privateKey: string) => {
